...
def compute_cost(parameters: np.ndarray, input_layer_nodes_size: int,
    ...
    theta = reshape_1d_parameter_array_to_respective_two_2d_arrays(parameters,
                                                                   input_layer_nodes_size, hidden_layer_nodes_size,
                                                                   num_labels)
...
def compute_gradients_with_back_propagation(parameters: np.ndarray, input_layer_nodes_size: int,
    ...
    theta_1_grad = del_theta_1 / number_of_training_samples + theta[
        1] * lambda_for_regularization / number_of_training_samples
    theta_0_grad = del_theta_0 / number_of_training_samples + theta[
        0] * lambda_for_regularization / number_of_training_samples
...
def minimize_cost_and_find_theta_with_optimization(nodes_in_input_layer,
                                                   nodes_in_output_layer,
                                                   X: np.ndarray,
                                                   y: np.ndarray,
                                                   num_of_classes,
                                                   lambda_for_regularization,
                                                   algo: OptimizationAlgo) -> tuple():
    """
    ...
    initial_theta_1 = randomly_initialize_parameters(nodes_in_output_layer, nodes_in_input_layer + 1)
    initial_theta_2 = randomly_initialize_parameters(num_of_classes, nodes_in_output_layer + 1)

    initial_parameters = np.hstack((initial_theta_1.transpose().flatten(),
                                    initial_theta_2.transpose().flatten()))
    ...
    if algo == OptimizationAlgo.MINIMIZE:
        ...
        result = so.minimize(fun=compute_cost(),
                             ...
                             args=(nodes_in_input_layer, nodes_in_output_layer, X, y, num_of_classes,
                                   lambda_for_regularization),
                             jac=compute_gradients_with_back_propagation)
        result = result.x

    if algo == OptimizationAlgo.FMIN_CG:
        result = so.fmin_cg(f=compute_cost,
                            x0=initial_parameters,
                            fprime=compute_gradients_with_back_propagation,
                            args=(nodes_in_input_layer, nodes_in_output_layer, X, y, num_of_classes,
                                  lambda_for_regularization),
                            maxiter=50)
